...
for (let i = 0; i < PSSR.length; i++) {
    ...
    const checkAllLoaded = () => {
        const partnersImages = document.getElementsByClassName('notEasyLoad')
        let allLoaded = true;
        for (let i = 0; i < partnersImages.length; i++) {
            if (!partnersImages[i].complete) {
                console.log(`${partnersImages[0].src} пока не загружено`);
                allLoaded = false;
            }
        }
        if (allLoaded) {
